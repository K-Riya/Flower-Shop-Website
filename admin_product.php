<?php
    ...
    if (isset($_POST['add_product']))
    {
        ...
        if(mysqli_num_rows($select_product_name) > 0)
        {
            ...
            if($insert_product)
            {
                ...
                {
                    $message[] = 'product size is too large';
                }
                else{
 ...
 if(isset($_POST['update_product'])){
    ...
    $update_query = mysqli_query($conn,"UPDATE `products` SET id='$update_p_id',name='$update_p_name',price='$update_p_price',product_detail='$update_p_detail',image='$update_p_image' WHERE id='$update_p_id'") or die("query failed");

    if($update_query){
        move_uploaded_file($update_p_image_tmp_name,$upate_p_image_folder);
